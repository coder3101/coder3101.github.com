...
From our analogy of weights as pipes when `x1, x2, x3` flow through the weights `w1, w2, w3` the outputs that will reach to the neuron are denoted as $w_{out}^1$ , $w_{out}^2$ , $w_{out}^3$ respectively. The above mathematical calculations denote these operations.
